...
const MOCK_COURSES = [
  ...
  },
  {
    ...
    description:
      "Сделайте первое впечатление незабываемым — заговорите на хинди!",
...
export const Courses = () => {
  ...
  const filterInitialValues = Object.fromEntries(
    Object.keys({ ...formatFilter, ...forWhomFilter }).map((key) => [
      key,
      false,
    ]),
  );
                ...
                ({ id, img_name, title, description, tags, slug }) => (
                  <Link to={`/courses/${slug}`} key={id}>
                    ...
                    >
                      <Wrapper
                        wrapperType="image"
                        className={styles.courses__wrapper_card_image}
                      >
                        <img
                      ...
                      <div className={styles.courses__wrapper_card_text}>
                        <Text
                          size="text-h2"
                          weight="font-bold"
                          className={styles.courses__wrapper_card_title}
                        >
                          {title}
                        </Text>
                        <div className={styles.courses__wrapper_card_desc}>
                          {truncateTextWithEllipsis(description ?? "", 120)}
                        </div>
