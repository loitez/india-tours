...
export const Swiper = ({slides, loop = true, autoPlay = true, autoPlayTime = 2000}) => {
    ...
    return (
        <div className={styles.swiper}>
            <div className={styles.swiper__cards} style={{
                transform: `translateX(-${currentSlide * 100}%)`, // Перемещаем слайды
                transition: currentSlide === 0 || currentSlide === slides.length + 1 ? 'none' : 'transform 0.5s ease',
            }}>
                {extendedSlides.map((slide, index) => (
                    <div className={styles.swiper__card} key={index} style={{ transform: `translateX(-${slide * 100}%)` }}>
