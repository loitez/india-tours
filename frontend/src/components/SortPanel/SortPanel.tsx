...
export const SortPanel = <T extends { title: string },>({items, onSortClick}: SortPanelProps<T>) => {
    const [alphabetSort, setAlphabetSort] = useState<boolean>(false)

    const onAlphabetSortButtonClick = () => {
        const sortedItems = [...items].sort((a, b) => {
            return alphabetSort ? b.title.localeCompare(a.title) : a.title.localeCompare(b.title);
        });
    ...
    return (
        <div className={styles.sort}>
            <Button version="invisible" onClick={onAlphabetSortButtonClick}>
                <Benefit icon={AlphabetIconSort[String(alphabetSort)]} className={styles.sort__item}>
